...
| `trading.pumpswap_read_timeout_ms`         | `2000`          | Per-call deadline on a single vault read. Distinct from `price_poll_interval_ms` (pause between polls): this is the budget for a SINGLE poll. Calls slower than this are killed and counted as failures. 1500-2000ms recommended; tighter risks self-induced failure storms when many sessions poll concurrently. |
